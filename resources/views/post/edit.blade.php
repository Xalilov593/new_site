                        ...
                        <div class="card-body">
                            <form action="{{route('posts.update', ['post'=>$post->id])}}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <input class="form-control" type="text"  name="content" value="{{isset($post) ? ($post->content) :  ''}}">
                                        @error('content')
                                            <span>{{$message}}</span>
                                        @enderror
                                    </div>
                                ...
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <select class="form-control selectric" name="second_category_id">
                                            @foreach($second_categories as $second_category)
                                                <option value="{{$second_category->id}}">{{$second_category->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
